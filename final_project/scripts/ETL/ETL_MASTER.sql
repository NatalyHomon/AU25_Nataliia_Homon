...
CREATE OR REPLACE PROCEDURE bl_cl.prc_run_etl_master_3nf(
...
AS $$
DECLARE
    v_proc   text := 'bl_cl.prc_run_etl_master_3nf';
...
BEGIN
    ...
    CALL bl_cl.pr_log_write(
        ...
        p_status         := 'START',
        p_rows_affected  := 0,
        p_message        := 'ETL master started (SA -> MAP -> 3NF )',
        p_sqlstate       := NULL,
...
CREATE OR REPLACE PROCEDURE bl_cl.prc_run_etl_master_dm(
     p_full_reload boolean DEFAULT false,
     p_months_back int     DEFAULT 3,
     p_run_id      uuid    DEFAULT NULL
)
...
AS $$
DECLARE
    v_proc   text := 'bl_cl.prc_run_etl_master_dm';
    v_run_id uuid := COALESCE(p_run_id, gen_random_uuid());
BEGIN
    -- MASTER START
    CALL bl_cl.pr_log_write(
        ...
        p_status         := 'START',
        p_rows_affected  := 0,
        p_message        := 'ETL master started ( 3NF -> DM)',
        p_sqlstate       := NULL,
        p_source_system  := 'bl_cl',
        p_source_entity  := 'ETL_MASTER',
        p_run_id         := v_run_id
    ...
    CALL bl_cl.pr_load_dim_dates_day_dm_simple (v_run_id);
	CALL bl_cl.pr_load_dim_dates_day_dm_simple (v_run_id);
...
-- incremental default 3NF
CALL bl_cl.prc_run_etl_master_3nf();

-- full reload 3NF:
CALL bl_cl.prc_run_etl_master_3nf(true);
...
-- more/less months
CALL bl_cl.prc_run_etl_master_dm(false, 6);
